    ...
    if (drih>0)
    {
        ...
        if (drisize)
        {
            ...
            if (finp != NULL)
            {
                ...
                strcat(dstr, qhm.toLatin1().constData());
...
drimgWidget::drimgWidget(QWidget* parent) :
    QWidget(parent),
    ...
    if (!act)  {
        ...
        else
        {
            ...
            if (geohit) {
                ...
                ui->edSecTr->setText(QString::number(sectr));

                ui->edHeads->setText( QString::number(heads));

                cylc = SecCnt/(sectr*heads) ;
                ui->edCyl->setText( QString::number(cylc));
        ...
        { ui->inf3Label->setText(" "); // Clear selected sec count message
            ...
            ui->secofEdit->setText(QString::number(OffFS));

            ui->seccnEdit->setText(QString::number(ChaSecCnt));
        }
...
void drimgWidget::openIfButton_clicked()
{
    if (!act)
    {
        ...
        if (form==0)
            fileName = QFileDialog::getOpenFileName(this, "File to Load", QString::null, "*.raw *.img|RAW & IMG image files\n*.*|All files");
        else
            fileName = QFileDialog::getOpenFileName(this, "File to load", QString::null, "*.hdf|Hdf image files\n*.*|All files");

        if( !fileName.isEmpty() )
        {

            fout = fopen(fileName.toLatin1().constData(),"rb");

            if (fout == NULL) {
                QMessageBox::warning(this, "File open error ", "Damn!" );
                return;
            ...
            strcpy(loadedF, fileName.toLatin1().constData());
            ...
            ui->edHeads->setText( QString::number( heads) );
            ui->edCyl->setText( QString::number( cylc) );
            ui->seccnEdit->setText( QString::number( SecCnt) );
            ...
            if (matc == 16) {
                ...
                else { ui->inf2Label->setText( PLUSIIS);
                    goto sethdf512 ; }
...
sethdf256:         ui->h256RB->setChecked(1);
            ui->hdfRB->setChecked(0);

            goto removh ;

sethdf512:   if ( form ) {  ui->h256RB->setChecked(0);
...
removh:
            ...
            ui->inf3Label->setText(QString("Sector count: %1").arg(SecCnt));
...
void drimgWidget::readButton_clicked()
{
    ...
    if (!act)
    {
        ...
        OffFS = ui->secofEdit->text().toULong();

        SecCnt = ui->seccnEdit->text().toULong();
        ...
        if ( selected == 99 ) { //Special case - hdf to raw or vice versa
            ...
            if ( (form==0) & (Fsub) ) // Swap L/H and save in new file
            {
                ...
                fileName = QFileDialog::getSaveFileName(0, i18n("*.raw|Raw image files\n*.img|Img image files\n*.*|All files"), this, i18n("File to save"));
